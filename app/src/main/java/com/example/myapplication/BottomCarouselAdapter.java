...
public class BottomCarouselAdapter extends RecyclerView.Adapter<BottomCarouselAdapter.VH> {
    ...
    @Override public void onBindViewHolder(@NonNull VH h, int position) {
        ...
        // Load colors
        int colorRed = ContextCompat.getColor(h.itemView.getContext(), R.color.red);
        // Dark gray for unselected to be visible on white
        int colorUnselected = 0xFF757575; 
        
        // Set background to white (or transparent) to match the new white footer theme
        // Using white allows the dark red text to be visible
        h.card.setCardBackgroundColor(0xFFFFFFFF);

        h.title.setTextColor(selected ? colorRed : colorUnselected);
        h.icon.setColorFilter(selected ? colorRed : colorUnselected);
